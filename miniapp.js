...
/** Read lastCheckIn timestamp from chain. Tries CHECKIN_ADDR first (where check-in is written), then LEADERBOARD_ADDR. */
window.baseInvadersGetLastCheckIn = async function (address) {
    ...
    for (const rpcUrl of BASE_RPC_URLS) {
        try {
            ...
            let ts = 0n;
            try {
                ts = await client.readContract({ address: CHECKIN_ADDR, abi, functionName: 'lastCheckIn', args: [address] });
            } catch (e) {
                ts = await client.readContract({ address: LEADERBOARD_ADDR, abi, functionName: 'lastCheckIn', args: [address] });
            }
            return ts ?? 0n;
...
window.baseInvadersGetLeaderboard = async function () {
    ...
    for (const rpcUrl of BASE_RPC_URLS) {
        try {
            ...
            const CHECKIN_ABI_READ = parseAbi(['function lastCheckIn(address) view returns (uint256)']);
            const withCheckIn = await Promise.all(arr.map(async (row) => {
                const player = row[0];
                let checkInTs = 0n;
                try {
                    checkInTs = await client.readContract({ address: CHECKIN_ADDR, abi: CHECKIN_ABI_READ, functionName: 'lastCheckIn', args: [player] });
                } catch (e) {
                    try {
                        checkInTs = await client.readContract({ address: LEADERBOARD_ADDR, abi: LEADERBOARD_ABI, functionName: 'lastCheckIn', args: [player] });
                    } catch (e2) { /* neither has lastCheckIn */ }
                }
